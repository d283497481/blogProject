import { Body, Controller, Delete, Get, HttpException, HttpStatus, Ip, Post, Query } from '@nestjs/common';
...
export class ScatteredController {
  constructor(private readonly blogService: ScatteredService) {}
   ...
   */
  @Get('/list')
  async getBlogs(@Query() query: blogList, @Ip() ip: string) {
  ...
  async findAll(@Body() param: createPostsDto) {
    if ('title' in param && 'txt' in param ) {
